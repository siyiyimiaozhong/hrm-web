<template>
  <div class="usersApprovalsContainer">
        ...
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column label="操作">
            <template slot-scope="item">
              <el-button
                v-show="tabLab == 'launch' && (item.row.stateOfApproval == '待审批' || item.row.stateOfApproval == '已驳回')"
                size="mini" type="primary" @click="clickCancel(item.row.id)">撤销
              </el-button>
              <el-button v-show="tabLab == 'approvals' && item.row.currentApproverId == userId" size="mini"
                         type="primary" @click="clickPass(item.row.id)">通过
              </el-button>
              <el-button v-show="tabLab == 'approvals' && item.row.currentApproverId == userId" size="mini"
                         type="primary" @click="clickBack(item.row.id)">驳回
              </el-button>
              <el-button size="mini" type="primary" @click="clickDetail(item.row.id,item.row.approvalType)">查看
              </el-button>
    ...
    <el-dialog :title="topLabel" :visible.sync="centerDialogVisible" width="50%" left>
      ...
      <Quit v-on:closeDialog="closeDialog" v-show="seeState === 'quit'" :selectedId="selectedId" :tabLab='tabLab'
            ref="quit"/>
      <!-- <Examine v-show="seeState == 'examine'" /> -->
      <Leave v-show="seeState === 'leave'" :selectedId="selectedId" :tabLab='tabLab' ref="leave"
             v-on:closeDialog="closeDialog"/>
      <Overtime v-show="seeState === 'overtime'" :selectedId="selectedId" :tabLab='tabLab' ref="overtime"
                v-on:closeDialog="closeDialog"/>
...
<script>
  import {approvalsDel, approvalsList, approvalsPass, approvalsReject} from '@/api/hrm/approvalsApi'
