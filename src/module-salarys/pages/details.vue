  ...
  <div class="detailsContainer">
    <div class="contLeft">
      <div class="topTit">
        ...
        <div class="info">
          <p class="name"><strong> {{formLabelAlign.username}} </strong><span
            :class="dutyStatus">{{dutyStatusTxt}}</span></p>
          <p class="time">入职时间: {{formLabelAlign.timeOfEntry}} 最新工资基数： {{formLabelAlign.latestSalaryBase}}
          <ul><span class='more'>?</span>
            <li>按月计薪员工为所有调薪后的基本工资、岗位工资合计（包括税前、税后）；按日计薪员工为基本工资*21.75+∑岗位工资项</li>
          </ul>
          </p>
          <p>最新 / 当月基本工资基数: {{formLabelAlign.basicWageBaseForTheLatestMonth}} /
            {{formLabelAlign.basicWageBaseForThatMonth}}</p>
          <p>最新 / 当月基本工资基数: {{formLabelAlign.salaryBaseForTheLatestMonth}} /
            {{formLabelAlign.salaryBaseForTheMonth}}</p>
        </div>
      </div>
      <div>
        <div class="social">
          ...
          <div class="table">
            ...
            <div class="tabRow">
              <div>交通补助</div>
              <div> {{formLabelAlign.transportationSubsidyAmount}}</div>
              <div>通讯补助</div>
              <div> {{formLabelAlign.communicationSubsidyAmount}}</div>
            </div>
            <div class="tabRow">
              <div>午餐补助</div>
              <div> {{formLabelAlign.lunchAllowanceAmount}}</div>
              <div>住房补助</div>
              <div> {{formLabelAlign.housingSubsidyAmount}}</div>
            </div>
          </div>
        </div>
        <div class="fund">
          <div class="title"><strong>社保公积金</strong> <span>企业 {{computeCompanyTotal}}</span> <span>个人 {{computePersonalTotal}}</span>
          </div>
          <div class="table">
            <div class="tabTit">
              <div>缴费项目</div>
            ...
            <div class="tabRow">
              <div>社保</div>
              <div> {{formLabelAlign.socialSecurityBase}}</div>
              <div>{{formLabelAlign.socialSecurityCompanyBase}}</div>
              <div> {{formLabelAlign.socialSecurityPersonalBase}}</div>
            </div>
            <div class="tabRow">
              <div>公积金</div>
              <div> {{formLabelAlign.providentFundBase}}</div>
              <div>{{formLabelAlign.providentFundCompanyBase}}</div>
              <div> {{formLabelAlign.providentFundPersonalBase}}</div>
            </div>
            <div class="tabRow">
              <div>缴费合计</div>
              <div></div>
              <div>{{computeCompanyTotal}}</div>
              <div> {{computePersonalTotal}}</div>
            </div>
          </div>
        </div>
        <div class="formTable">
          <el-form :label-position="'left'" label-width="180px" :model="formLabelAlign">
            <el-form-item label="实际出勤天数（正式）：" style="width: 50%;">
              <el-input v-model="formLabelAlign.actualAttendanceDaysAreOfficial" placeholder="0"
