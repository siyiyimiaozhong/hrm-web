<template>
  <div class="listContainer">
    <div class="cont-top-box">
      <div class="top-lable">
        <div class="careful-lab">
          ...
          本月{{tips.dateRange}}：入职 {{tips.worksCount}} 离职 {{tips.leavesCount}} 调薪 {{tips.adjustCount}} 未定薪
          {{tips.unGradingCount}}
        </div>
        <div class="lable-tit">
          <router-link :to="{ 'path':'/salarys/setting'}">设置</router-link>
          <router-link :to="{'path':'/salarys/historicalArchiving'}">历史归档</router-link>
          <router-link :to="{'path':'/salarys/monthStatement',query:{'yearMonth':yearMonth}}">{{tips.strMonth}}月报表
          </router-link>
        </div>
      </div>
    </div>
    <div class="approvalsContent">
      ...
      <div class="content">
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column prop="wageBase" label="工资基数"></el-table-column>
          <el-table-column prop="subsidyScheme" label="津贴方案"></el-table-column>
          <el-table-column label="操作" width="260">
            <template slot-scope="scope" style>
              <el-button
                size="mini"
                type="primary"
                v-if="scope.row.isFixed == true"
    ...
    <el-dialog :title="topLabel" :visible.sync="centerDialogVisible" width="50%" left>
      <ChangeSalary v-show="seeState == 'changeSalary'" v-on:onDialogCancel="onDialogCancel"
...
<script>
  import {getSalary, getSalarysList, getTips} from '@/api/hrm/salarysApi'
  import {list as getDepartmentList} from '@/api/base/departments'
