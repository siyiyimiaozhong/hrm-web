  ...
  <div class="cont-top-box">
    <div class="top-lable">
      <div class="careful-lab">
        <i class="el-icon-info"></i>本月{{tips.dateRange}}：社保在缴 {{tips.socialSecurityCount}} 公积金在缴
        {{tips.providentFundCount}} 增员 {{tips.newsCount}} 减员 {{tips.reducesCount}} 入职 {{tips.worksCount}} 离职
        {{tips.leavesCount}}
      </div>
      <div class="lable-tit">
        <el-input
    ...
    <div>
      <li>
        ...
        <el-checkbox-group v-model="departmentChecks" style="display:inline-block">
          <el-checkbox v-for="item in departmentList" :label="item.id" :key="item.id" @change="checkChange">
            {{item.name}}
          </el-checkbox>
        </el-checkbox-group>
      </li>
      ...
      <li>
        ...
        <el-checkbox-group v-model="socialSecurityChecks" style="display:inline-block">
          <el-checkbox v-for="item in cityList" :label="item.id" :key="item.id" @change="checkChange">{{item.name}}
          </el-checkbox>
        </el-checkbox-group>
      </li>
      <br>
